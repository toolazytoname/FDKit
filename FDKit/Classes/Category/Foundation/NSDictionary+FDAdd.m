...
FDSYNTH_DUMMY_CLASS(NSDictionary_FDAdd)


@interface _FDXMLDictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _FDXMLDictionaryParser {
...
- (void)textEnd {
    _text = _text.fd_stringByTrim.mutableCopy;
    if (_text.length) {
...
@implementation NSDictionary (FDAdd)

+ (NSDictionary *)fd_dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
...
+ (NSDictionary *)fd_dictionaryWithPlistString:(NSString *)plist {
    if (!plist) return nil;
    NSData* data = [plist dataUsingEncoding:NSUTF8StringEncoding];
    return [self fd_dictionaryWithPlistData:data];
}
...
- (NSArray *)fd_allValuesSortedByKeys {
    NSArray *sortedKeys = [self fd_allKeysSorted];
...
+ (NSDictionary *)fd_dictionaryWithXML:(id)xml {
    _FDXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_FDXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_FDXMLDictionaryParser alloc] initWithData:xml];
    }
...
- (NSNumber *)fd_numberValueForKey:(NSString *)key default:(NSNumber *)def {
    if (!key) return def;
    id value = self[key];
    if (!value || value == [NSNull null]) return def;
    if ([value isKindOfClass:[NSNumber class]]) return value;
...
- (NSString *)fd_stringValueForKey:(NSString *)key default:(NSString *)def {
    if (!key) return def;
    id value = self[key];
    if (!value || value == [NSNull null]) return def;
...
@implementation NSMutableDictionary (FDAdd)

+ (NSMutableDictionary *)fd_dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
...
+ (NSMutableDictionary *)fd_dictionaryWithPlistString:(NSString *)plist {
    if (!plist) return nil;
    NSData* data = [plist dataUsingEncoding:NSUTF8StringEncoding];
    return [self fd_dictionaryWithPlistData:data];
}

- (id)fd_popObjectForKey:(id)aKey {
    if (!aKey) return nil;
    id value = self[aKey];
