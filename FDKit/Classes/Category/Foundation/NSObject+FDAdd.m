...
FDSYNTH_DUMMY_CLASS(NSObject_FDAdd)


@implementation NSObject (FDAdd)
...
[NSObject fd_setInv:inv withSig:sig andArgs:args]; \
va_end(args);

- (id)fd_performSelectorWithArgs:(SEL)sel, ...{
    INIT_INV(sel, nil);
    [inv invoke];
    return [NSObject fd_getReturnFromInv:inv withSig:sig];
}

- (void)fd_performSelectorWithArgs:(SEL)sel afterDelay:(NSTimeInterval)delay, ...{
    INIT_INV(delay, );
    [inv retainArguments];
    [inv performSelector:@selector(invoke) withObject:nil afterDelay:delay];
}

- (id)fd_performSelectorWithArgsOnMainThread:(SEL)sel waitUntilDone:(BOOL)wait, ...{
...
#pragma clang diagnostic pop
    ...
    return wait ? [NSObject fd_getReturnFromInv:inv withSig:sig] : nil;
}

- (id)fd_performSelectorWithArgs:(SEL)sel onThread:(NSThread *)thr waitUntilDone:(BOOL)wait, ...{
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wvarargs"
    INIT_INV(wait, nil);
#pragma clang diagnostic pop
...
- (void)fd_performSelectorWithArgsInBackground:(SEL)sel, ...{
    INIT_INV(sel, );
    [inv retainArguments];
...
+ (id)fd_getReturnFromInv:(NSInvocation *)inv withSig:(NSMethodSignature *)sig {
...
+ (void)fd_setInv:(NSInvocation *)inv withSig:(NSMethodSignature *)sig andArgs:(va_list)args {
...
#define case_size(_size_) \
    ...
    [inv setArgument:&arg atIndex:index]; \
            ...
            case_size(61) case_size(62) case_size(63) case_size(64)
                ...
                NSLog(@"FDCategories performSelectorWithArgs unsupported type:%s (%lu bytes)",
...
- (void)fd_performSelector:(SEL)selector afterDelay:(NSTimeInterval)delay {
    [self performSelector:selector withObject:nil afterDelay:delay];
}

+ (BOOL)fd_swizzleInstanceMethod:(SEL)originalSel with:(SEL)newSel {
...
+ (BOOL)fd_swizzleClassMethod:(SEL)originalSel with:(SEL)newSel {
...
- (void)fd_setAssociateValue:(id)value withKey:(void *)key {
    objc_setAssociatedObject(self, key, value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)fd_setAssociateWeakValue:(id)value withKey:(void *)key {
    objc_setAssociatedObject(self, key, value, OBJC_ASSOCIATION_ASSIGN);
}

- (void)fd_removeAssociatedValues {
    objc_removeAssociatedObjects(self);
}

- (id)fd_getAssociatedValueForKey:(void *)key {
    return objc_getAssociatedObject(self, key);
}

+ (NSString *)fd_className {
...
- (id)fd_deepCopyWithArchiver:(Class)archiver unarchiver:(Class)unarchiver {
    id obj = nil;
    @try {
