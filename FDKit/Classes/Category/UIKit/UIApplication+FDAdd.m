...
FDSYNTH_DUMMY_CLASS(UIApplication_FDAdd)

#define kNetworkIndicatorDelay (1/30.0)
@interface _FDUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _FDUIApplicationNetworkIndicatorInfo
@end


@implementation UIApplication (FDAdd)

- (NSURL *)fd_documentsURL {
    return [[[NSFileManager defaultManager]
             URLsForDirectory:NSDocumentDirectory
             inDomains:NSUserDomainMask] lastObject];
}

- (NSString *)fd_documentsPath {
...
- (NSString *)fd_libraryPath {
    return [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject];
}

- (BOOL)fd_isPirated {
    if ([[UIDevice currentDevice] fd_isSimulator]) return YES; // Simulator is not from appstore
    ...
    if (![self _fd_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _fd_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_fd_fileExistInMainBundle:(NSString *)name {
    NSString *bundlePath = [[NSBundle mainBundle] bundlePath];
    NSString *path = [NSString stringWithFormat:@"%@/%@", bundlePath, name];
    return [[NSFileManager defaultManager] fileExistsAtPath:path];
}

- (NSString *)fd_appBundleName {
    return [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleName"];
}

- (NSString *)fd_appBundleID {
    return [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleIdentifier"];
}

- (NSString *)fd_appVersion {
    return [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
}

- (NSString *)fd_appBuildVersion {
...
FDSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _FDUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _FDUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_FDUIApplicationNetworkIndicatorInfo new];
...
+ (UIApplication *)fd_sharedExtensionApplication {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    return [self fd_isAppExtension] ? nil : [UIApplication performSelector:@selector(sharedApplication)];
