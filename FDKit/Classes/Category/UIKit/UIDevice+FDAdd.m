...
FDSYNTH_DUMMY_CLASS(UIDevice_FDAdd)


@implementation UIDevice (FDAdd)
...
- (BOOL)fd_isJailbroken {
    if ([self fd_isSimulator]) return NO; // Dont't check simulator
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString fd_stringWithUUID]];
...
- (NSString *)fd_ipAddressWIFI {
    return [self ipAddressWithIfaName:@"en0"];
}

- (NSString *)fd_ipAddressCell {
...
static uint64_t yy_net_counter_get_by_type(yy_net_interface_counter *counter, FDNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & FDNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & FDNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & FDNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & FDNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & FDNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & FDNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
...
- (NSString *)fd_machineModelName {
    static dispatch_once_t one;
    static NSString *name;
    dispatch_once(&one, ^{
        NSString *model = [self fd_machineModel];
        if (!model) return;
        NSDictionary *dic = @{
...
- (NSDate *)fd_systemUptime {
    NSTimeInterval time = [[NSProcessInfo processInfo] systemUptime];
    return [[NSDate alloc] initWithTimeIntervalSinceNow:(0 - time)];
}

- (int64_t)fd_diskSpace {
    NSError *error = nil;
...
- (int64_t)fd_diskSpaceUsed {
    int64_t total = self.fd_diskSpace;
    int64_t free = self.fd_diskSpaceFree;
...
- (int64_t)fd_memoryTotal {
    int64_t mem = [[NSProcessInfo processInfo] physicalMemory];
    if (mem < -1) mem = -1;
    return mem;
}

- (int64_t)fd_memoryUsed {
...
- (NSUInteger)fd_cpuCount {
    return [NSProcessInfo processInfo].activeProcessorCount;
}

- (float)fd_cpuUsage {
    float cpu = 0;
    NSArray *cpus = [self fd_cpuUsagePerProcessor];
...
- (NSArray *)fd_cpuUsagePerProcessor {
