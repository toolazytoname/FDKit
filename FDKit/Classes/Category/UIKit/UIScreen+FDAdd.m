...
FDSYNTH_DUMMY_CLASS(UIScreen_FDAdd);


@implementation UIScreen (FDAdd)

+ (CGFloat)fd_screenScale {
...
- (CGRect)fd_currentBounds {
    return [self fd_boundsForOrientation:[[UIApplication sharedApplication] statusBarOrientation]];
}
#endif

- (CGRect)fd_boundsForOrientation:(UIInterfaceOrientation)orientation {
...
- (CGSize)fd_sizeInPixel {
    CGSize size = CGSizeZero;
    
    if ([[UIScreen mainScreen] isEqual:self]) {
//        NSString *model = [UIDevice currentDevice].fd_machineModel;
        //数据不准，暂时注释掉
