...
@implementation FDBootingProtection

+ (BOOL)launchContinuousCrashProtect {
    NSAssert(repairBlock, @"repairBlock is nil!");
    if (Logger) Logger(@"FDBootingProtection: Launch continuous crash report");
    [self setIsFixing:NO];
    NSInteger launchCrashes = [self crashCount];
    // 上报
    if (launchCrashes >= kContinuousCrashOnLaunchNeedToReport) {
        if (Logger) Logger([NSString stringWithFormat:@"FDBootingProtection: App has continuously crashed for %@ times. Now synchronize uploading crash report and begin fixing procedure.", @(launchCrashes)]);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kCrashOnLaunchTimeIntervalThreshold * NSEC_PER_SEC)), dispatch_get_main_queue(), ^(void){
        // APP活过了阈值时间，重置崩溃计数
        if (Logger) Logger([NSString stringWithFormat:@"FDBootingProtection: long live the app ( more than %@ seconds ), now reset crash counts", @(kCrashOnLaunchTimeIntervalThreshold)]);
...
+ (BOOL)startupCrashForTest {
    if ([FDBootingProtection crashCount] >= kContinuousCrashOnLaunchNeedToFix) {
        return NO;
    }
