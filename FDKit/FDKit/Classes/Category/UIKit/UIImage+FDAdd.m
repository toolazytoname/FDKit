...
+ (UIImage *)fd_imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
...
+ (BOOL)fd_isAnimatedGIFData:(NSData *)data {
...
+ (BOOL)fd_isAnimatedGIFFile:(NSString *)path {
...
+ (UIImage *)fd_imageWithPDF:(id)dataOrPath {
    return [self _yy_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)fd_imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _yy_imageWithPDF:dataOrPath resize:YES size:size];
}

+ (UIImage *)fd_imageWithEmoji:(NSString *)emoji size:(CGFloat)size {
...
+ (UIImage *)fd_imageWithColor:(UIColor *)color {
    return [self fd_imageWithColor:color size:CGSizeMake(1, 1)];
}
...
+ (UIImage *)fd_imageWithSize:(CGSize)size drawBlock:(void (^)(CGContextRef context))drawBlock {
...
- (void)fd_drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
...
- (UIImage *)fd_imageByResizeToSize:(CGSize)size contentMode:(UIViewContentMode)contentMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    UIGraphicsBeginImageContextWithOptions(size, NO, self.scale);
...
- (UIImage *)fd_imageByInsetEdge:(UIEdgeInsets)insets withColor:(UIColor *)color {
...
- (UIImage *)fd_imageByRotate:(CGFloat)radians fitSize:(BOOL)fitSize {
...
- (UIImage *)fd_imageByRotateLeft90 {
    return [self fd_imageByRotate:DegreesToRadians(90) fitSize:YES];
}

- (UIImage *)fd_imageByRotateRight90 {
    return [self fd_imageByRotate:DegreesToRadians(-90) fitSize:YES];
}

- (UIImage *)fd_imageByRotate180 {
    return [self _yy_flipHorizontal:YES vertical:YES];
}

- (UIImage *)fd_imageByFlipVertical {
    return [self _yy_flipHorizontal:NO vertical:YES];
}

- (UIImage *)fd_imageByFlipHorizontal {
    return [self _yy_flipHorizontal:YES vertical:NO];
}

- (UIImage *)fd_imageByTintColor:(UIColor *)color {
    UIGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
    CGRect rect = CGRectMake(0, 0, self.size.width, self.size.height);
    [color set];
    UIRectFill(rect);
...
- (UIImage *)fd_imageByGrayscale {
    return [self fd_imageByBlurRadius:0 tintColor:nil tintMode:0 saturation:0 maskImage:nil];
}

- (UIImage *)fd_imageByBlurSoft {
    return [self fd_imageByBlurRadius:60 tintColor:[UIColor colorWithWhite:0.84 alpha:0.36] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)fd_imageByBlurLight {
    return [self fd_imageByBlurRadius:60 tintColor:[UIColor colorWithWhite:1.0 alpha:0.3] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)fd_imageByBlurExtraLight {
    return [self fd_imageByBlurRadius:40 tintColor:[UIColor colorWithWhite:0.97 alpha:0.82] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)fd_imageByBlurDark {
    return [self fd_imageByBlurRadius:40 tintColor:[UIColor colorWithWhite:0.11 alpha:0.73] tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}

- (UIImage *)fd_imageByBlurWithTint:(UIColor *)tintColor {
    ...
    return [self fd_imageByBlurRadius:20 tintColor:effectColor tintMode:kCGBlendModeNormal saturation:-1.0 maskImage:nil];
}

- (UIImage *)fd_imageByBlurRadius:(CGFloat)blurRadius
                     tintColor:(UIColor *)tintColor
                      tintMode:(CGBlendMode)tintBlendMode
                    saturation:(CGFloat)saturation
                     maskImage:(UIImage *)maskImage {
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+FDAdd error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+FDAdd error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+FDAdd error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            NSLog(@"UIImage+FDAdd error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            NSLog(@"UIImage+FDAdd error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
