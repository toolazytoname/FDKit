...
@class BPWSKUDataFilter;
@class BPWSKUProperty;
...
//属性种类个数
- (NSInteger)numberOfSectionsForPropertiesInFilter:(BPWSKUDataFilter *)filter;

/*
 * 每个种类所有的的属性值
 * 这里不关心具体的值，可以是属性ID, 属性名，字典、model
 */
- (NSArray *)filter:(BPWSKUDataFilter *)filter propertiesInSection:(NSInteger)section;

//满足条件 的 个数
- (NSInteger)numberOfConditionsInFilter:(BPWSKUDataFilter *)filter;

/*
 * 对应的条件式
 * 这里条件式的属性值，需要和propertiesInSection里面的数据类型保持一致
 */
- (NSArray *)filter:(BPWSKUDataFilter *)filter conditionForRow:(NSInteger)row;

//条件式 对应的 结果数据（库存、价格等）
- (id)filter:(BPWSKUDataFilter *)filter resultOfConditionForRow:(NSInteger)row;

@end

@interface BPWSKUDataFilter : NSObject
...
@interface BPWSKUCondition :NSObject

@property (nonatomic, strong) NSArray<BPWSKUProperty *> *properties;
...
@interface BPWSKUProperty :NSObject
