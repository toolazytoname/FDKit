...
@implementation BPASkinItemView
...
- (void)_startAnimation {
    ...
    if ([BPASkinManager shouldPerformScaleAnimation]) {
        [animationArray BPT_addObj:[self.imageView.layer bpa_scaleAnamation]];
    }
    if ([BPASkinManager shouldPerformMoveAnimation]) {
        [animationArray BPT_addObj:[self.imageView.layer bpa_upAnimationWithStartY:self.BPT_height/2 + imageTop]];
    }
    [self.imageView.layer bpa_addAnimations:animationArray];
}
...
- (void)_resetBrotherCustomTabItemView {
    [self.superview.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:self.class] && obj != self) {
            [(BPASkinItemView *)obj setSelected:NO];;
        }
    }];
}
