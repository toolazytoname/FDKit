...
#define BPACustomMoveAnimation @"1"
#define BPACustomScaleAnimation @"2"

@interface BPASkinManager()<BPBGlobalEventDelegate>
@property (nonatomic, strong) BPAWebImageManager *imageManager;
@end

@implementation BPASkinManager
...
+ (BOOL)shouldPerformScaleAnimation {
    return [[self _animationArray] containsObject:BPACustomScaleAnimation];
}

+ (BOOL)shouldPerformMoveAnimation {
    return [[self _animationArray] containsObject:BPACustomMoveAnimation];
}
...
- (BPAWebImageManager *)imageManager {
    if (!_imageManager) {
        _imageManager = [[BPAWebImageManager alloc] init];
